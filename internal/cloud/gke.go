...
// gkeDetector resolves the GKE cluster link from the node providerID and the GCP
// metadata server.
type gkeDetector struct {
	metadataBaseURL string
}

// detect assembles /projects/<projectId>/locations/<location>/clusters/<clusterName>.
...
func (g gkeDetector) detect(ctx context.Context, providerID string) (string, error) {
	name, err := g.metadata(ctx, "/instance/attributes/cluster-name")
	if err != nil {
		return "", fmt.Errorf("querying GKE cluster-name: %w", err)
	}
	location, err := g.metadata(ctx, "/instance/attributes/cluster-location")
	if err != nil {
	...
	if projectID == "" {
		projectID, err = g.metadata(ctx, "/project/project-id")
		if err != nil {
...
// metadata performs a GET against the GCP metadata server and returns the trimmed body.
func (g gkeDetector) metadata(ctx context.Context, path string) (string, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, g.metadataBaseURL+path, nil)
	if err != nil {
