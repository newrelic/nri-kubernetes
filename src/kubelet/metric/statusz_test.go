...
func (m *mockHTTPGetterStatusz) Get(path string) (*http.Response, error) {
...
func (m *mockHTTPGetterStatusz) GetURI(uri url.URL) (*http.Response, error) {
	return &http.Response{
		StatusCode: 200,
		Body:       io.NopCloser(bytes.NewBufferString(m.response)),
	}, nil
...
func TestKubeletStatuszFetcher_JSONFormat_Kubernetes135(t *testing.T) {
  ...
  "componentStatuses": [
	...
	rawResponse, ok := nodeMetrics["kubeletStatuszRawResponse"]
	assert.True(t, ok, "raw response should exist")
