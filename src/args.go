...
func (args *argumentList) multiKSMDiscoverer(logger log.Logger, nodeIP string) (client.MultiDiscoverer, error) {
	k8sClient, err := client.NewKubernetes( /* tryLocalKubeconfig */ false)
	if err != nil {
		return nil, fmt.Errorf("initializing Kubernetes client: %w", err)
	}
	...
	if args.KubeStateMetricsPodLabel == "" {
		return nil, fmt.Errorf("multi KSM discovery set without a KUBE_STATE_METRICS_POD_LABEL")
	}
