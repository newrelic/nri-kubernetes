...
func run() error {
	...
	if err != nil {
		return fmt.Errorf("building kubelet discoverer: %w", err)
	}
	...
	if err != nil {
		logger.Warnf("Could not find default network interface: %v", err)
	}
	...
	if !args.DisableKubeStateMetrics {
		...
		if args.DistributedKubeStateMetrics {
			ksmDiscoverer, err := args.multiKSMDiscoverer(logger, kubeletNodeIP)
			if err != nil {
			...
			if err != nil {
				return fmt.Errorf("getting KSM discoverer: %v", err)
			}
