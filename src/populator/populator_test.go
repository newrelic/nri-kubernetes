...
func TestIntegrationPopulator_KSMAnnotationLabel_Exported(t *testing.T) { //nolint: funlen
	rawGroups := definition.RawGroups{
		"deployment": {
			"my-deployment": {
				...
				"kube_deployment_annotations": prometheus.Metric{
					Labels: prometheus.Labels{
						"annotation_description": "my-test-deployment",
					},
				},
			},
		},
	...
	specGroups := definition.SpecGroups{
		"deployment": {
			...
			Specs: []definition.Spec{
				{Name: "label.*", ValueFunc: prometheus.FromMetricWithPrefixedLabels("kube_deployment_labels", "label"), Type: metric.ATTRIBUTE},
				{Name: "annotation.*", ValueFunc: prometheus.FromMetricWithPrefixedLabels("kube_deployment_annotations", "annotation"), Type: metric.ATTRIBUTE},
			},
		},
		"namespace": {
			IDGenerator: func(_, _ string, _ definition.RawGroups) (string, error) {
			...
			Specs: []definition.Spec{
				{Name: "label.*", ValueFunc: prometheus.FromMetricWithPrefixedLabels("kube_namespace_labels", "label"), Type: metric.ATTRIBUTE},
				{Name: "annotation.*", ValueFunc: prometheus.FromMetricWithPrefixedLabels("kube_namespace_annotations", "annotation"), Type: metric.ATTRIBUTE},
			},
		},
		"pod": {
			...
			Specs: []definition.Spec{
				{Name: "label.*", ValueFunc: prometheus.FromMetricWithPrefixedLabels("kube_pod_labels", "label"), Type: metric.ATTRIBUTE},
				{Name: "annotation.*", ValueFunc: prometheus.FromMetricWithPrefixedLabels("kube_pod_annotations", "annotation"), Type: metric.ATTRIBUTE},
			},
		},
	}

	// Create a new integration and run the populator.
	intgr, err := integration.New("test", "dev")
	require.NoError(t, err)

	populatorConfig := &definition.IntegrationPopulateConfig{
		Integration: intgr,
		ClusterName: "test-cluster",
		K8sVersion:  &version.Info{GitVersion: "v1.23.0"},
		Groups:      rawGroups,
		Specs:       specGroups,
		MsTypeGuesser: func(group string) (string, error) {
			return fmt.Sprintf("K8s%sSample", cases.Title(language.English).String(group)), nil
		},
	}

	_, errs := IntegrationPopulator(populatorConfig)
	assert.Empty(t, errs)

	// Assert that the entities were created with the correct labels and annotations.
	expected := map[string]map[string]interface{}{
		"my-deployment": {
			"label.app":              "my-app",
			"annotation.description": "my-test-deployment",
		},
		"my-namespace": {
			"label.env":        "dev",
			"annotation.owner": "test-team",
		},
		"my-pod": {
			"label.version":      "1.0",
			"annotation.release": "canary",
		},
	}

	var entities []*integration.Entity
	for _, e := range intgr.Entities {
		if _, ok := expected[e.Metadata.Name]; ok {
			entities = append(entities, e)
		}
	}

	require.Len(t, entities, 3, "expected 3 entities (deployment, namespace, pod, cluster)")

	for _, entity := range entities {
		expectedMetrics, ok := expected[entity.Metadata.Name]
		require.True(t, ok, "unexpected entity found: %s", entity.Metadata.Name)
		...
		for k, v := range expectedMetrics {
			...
			assert.True(t, ok, "metric %s not found for entity %s", k, entity.Metadata.Name)
			assert.Equal(t, v, metricValue, "metric value mismatch for %s on entity %s", k, entity.Metadata.Name)
		}
	}
}
