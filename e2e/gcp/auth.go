/*
	This file is a copy of the "k8s.io/client-go/plugin/pkg/client/auth/gcp" but with a minor but critical modification:
		- glog package import was replaced by stdlib log. This fixes the "flag redefined: log_dir" panic err
*/
...
func (c *commandTokenSource) parseTokenCmdOutput(output []byte) (*oauth2.Token, error) {
	...
	if t, err := time.Parse(c.timeFmt, expiryStr); err != nil {
		log.Printf("Failed to parse token expiry from %s (fmt=%s): %v", expiryStr, c.timeFmt, err)
	} else {
		expiry = t
	}
