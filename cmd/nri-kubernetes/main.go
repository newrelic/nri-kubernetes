...
func main() {
	...
	var nsFilter *discovery.NamespaceFilter
	...
	if c.Kubelet.Enabled {
		kubeletScraper, err = setupKubelet(c, clients, nsFilter)
		if err != nil {
	...
	if c.KSM.Enabled {
		ksmScraper, err = setupKSM(c, clients, nsFilter)
		if err != nil {
			logger.Errorf("setting up ksm scraper: %v", err)
			os.Exit(exitSetup)
		}
...
func setupKSM(c *config.Config, clients *clusterClients, nsFilter *discovery.NamespaceFilter) (*ksm.Scraper, error) {
...
func setupKubelet(c *config.Config, clients *clusterClients, nsFilter *discovery.NamespaceFilter) (*kubelet.Scraper, error) {
